...
class Mosaic(T.Transform):
    ...
    @staticmethod
    def _paste_masks_on_canvas(masks, offset_xy, canvas_height, canvas_width):
        if masks is None:
            return masks
        raw = masks.as_subclass(torch.Tensor) if hasattr(masks, 'as_subclass') else masks
        if raw.numel() == 0:
            return raw.new_zeros((0, canvas_height, canvas_width))
        ox, oy = int(offset_xy[0]), int(offset_xy[1])
        n, h, w = raw.shape[-3], raw.shape[-2], raw.shape[-1]
        canvas = raw.new_zeros((n, canvas_height, canvas_width))
        canvas[:, oy:oy + h, ox:ox + w] = raw
        return canvas

    @staticmethod
    def _cat_target_values(values, dim=0):
    ...
    def create_mosaic_from_cache(self, mosaic_samples, max_height, max_width):
        ...
        for i, sample in enumerate(mosaic_samples):
            ...
            if 'tooth_boxes' in target:
                target['tooth_boxes'] = target['tooth_boxes'] + offsets[i]
            if 'tooth_midline_boxes' in target:
                target['tooth_midline_boxes'] = target['tooth_midline_boxes'] + offsets[i]
            if 'tooth_corner_boxes' in target:
                target['tooth_corner_boxes'] = target['tooth_corner_boxes'] + offsets[i]
            if 'tooth_masks' in target:
                target['tooth_masks'] = self._paste_masks_on_canvas(
                    target['tooth_masks'], placement_offsets[i], canvas_height, canvas_width)
            mosaic_target.append(target)

        merged_target = {}
        for key in mosaic_target[0]:
            merged_target[key] = self._cat_target_values([target[key] for target in mosaic_target])
    ...
    def create_mosaic_from_dataset(self, images, targets, max_height, max_width):
        ...
        for key in targets[0]:
            if key in ('boxes', 'tooth_boxes', 'tooth_midline_boxes', 'tooth_corner_boxes'):
                values = [target[key] + offsets[i] for i, target in enumerate(targets)]
            elif key == 'tooth_masks':
                values = [self._paste_masks_on_canvas(target[key], placement_offsets[i], canvas_height, canvas_width)
                          for i, target in enumerate(targets)]
            else:
                values = [target[key] for target in targets]

            merged_target[key] = self._cat_target_values(values)
    ...
    def forward(self, *inputs):
        ...
        if 'tooth_boxes' in mosaic_target:
            mosaic_target['tooth_boxes'] = convert_to_tv_tensor(mosaic_target['tooth_boxes'], 'boxes', box_format='xyxy',
                                                                spatial_size=mosaic_image.size[::-1])
        if 'tooth_midline_boxes' in mosaic_target:
            mosaic_target['tooth_midline_boxes'] = convert_to_tv_tensor(mosaic_target['tooth_midline_boxes'], 'boxes', box_format='xyxy',
                                                                        spatial_size=mosaic_image.size[::-1])
        if 'tooth_corner_boxes' in mosaic_target:
            mosaic_target['tooth_corner_boxes'] = convert_to_tv_tensor(mosaic_target['tooth_corner_boxes'], 'boxes', box_format='xyxy',
                                                                       spatial_size=mosaic_image.size[::-1])
        if 'tooth_corner_boxes' in mosaic_target:
            mosaic_target['tooth_corner_boxes'] = convert_to_tv_tensor(mosaic_target['tooth_corner_boxes'], 'boxes', box_format='xyxy',
                                                                       spatial_size=mosaic_image.size[::-1])
        if 'tooth_masks' in mosaic_target:
            mosaic_target['tooth_masks'] = convert_to_tv_tensor(mosaic_target['tooth_masks'], 'masks')
