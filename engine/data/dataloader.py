"""
Anatomy-DETR dataloader utilities, built upon DEIM.
...
def generate_scales(base_size, base_size_repeat):
    base_size_repeat = 1 if base_size_repeat is None else base_size_repeat

    if isinstance(base_size, (list, tuple)):
        base_h, base_w = base_size
        ratio = base_w / base_h

        # 向上取整，保证最小尺寸不会低于设定的 0.8 倍
        min_h = math.ceil(base_h * 0.8 / 32) * 32

        hs = list(range(min_h, base_h, 32))
        hs += [base_h] * base_size_repeat

        scales = []
        for h in hs:
            w = round(h * ratio / 32) * 32
            w = min(w, base_w)
            scales.append((h, w))

        return scales

    min_size = math.ceil(base_size * 0.8 / 32) * 32
    scales = list(range(min_size, base_size, 32))
...
class BatchImageCollateFunction(BaseCollateFunction):
    ...
    def apply_mixup(self, images, targets):
        ...
        if random.random() < self.mixup_prob and self.mixup_epochs[0] <= self.epoch < self.mixup_epochs[-1]:
            ...
            for i in range(len(targets)):
                ...
                if 'tooth_boxes' in targets[i] and 'tooth_boxes' in shifted_targets[i]:
                    updated_targets[i]['tooth_boxes'] = torch.cat([targets[i]['tooth_boxes'], shifted_targets[i]['tooth_boxes']], dim=0)
                    updated_targets[i]['tooth_scores'] = torch.cat([targets[i]['tooth_scores'], shifted_targets[i]['tooth_scores']], dim=0)
                    updated_targets[i]['tooth_types'] = torch.cat([targets[i]['tooth_types'], shifted_targets[i]['tooth_types']], dim=0)

                    if 'tooth_rboxes' in targets[i] and 'tooth_rboxes' in shifted_targets[i]:
                        updated_targets[i]['tooth_rboxes'] = torch.cat(
                            [targets[i]['tooth_rboxes'], shifted_targets[i]['tooth_rboxes']], dim=0
                        )

                    if 'tooth_rbox_valid' in targets[i] and 'tooth_rbox_valid' in shifted_targets[i]:
                        updated_targets[i]['tooth_rbox_valid'] = torch.cat(
                            [targets[i]['tooth_rbox_valid'], shifted_targets[i]['tooth_rbox_valid']], dim=0
                        )

                    if 'tooth_masks' in targets[i] and 'tooth_masks' in shifted_targets[i]:
                        updated_targets[i]['tooth_masks'] = torch.cat([targets[i]['tooth_masks'], shifted_targets[i]['tooth_masks']], dim=0)

                    if 'tooth_midline_boxes' in targets[i] and 'tooth_midline_boxes' in shifted_targets[i]:
                        updated_targets[i]['tooth_midline_boxes'] = torch.cat([targets[i]['tooth_midline_boxes'], shifted_targets[i]['tooth_midline_boxes']], dim=0)
    ...
    def __call__(self, items):
        ...
        # #1 light optimization: tooth_masks are only needed to estimate rotated boxes, which already
        # happened inside BuildToothRBoxesFromMasks (drop_masks=True).  Defensively release any mask
        # that survived (e.g. keep_debug_masks or a reordered pipeline) so full-resolution masks never
        # inflate the batch or pin worker memory.  tooth_rboxes / tooth_rbox_valid are kept.
        for tg in targets:
            if 'tooth_masks' in tg:
                tg.pop('tooth_masks', None)
        ...
        if self.scales is not None and self.epoch < self.stop_epoch:
            ...
            images = F.interpolate(images, size=tuple(size), mode='bilinear', align_corners=False)

            if 'masks' in targets[0]:
                for tg in targets:
                    tg['masks'] = F.interpolate(tg['masks'], size=size, mode='nearest')
                raise NotImplementedError('')
