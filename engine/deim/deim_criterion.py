"""
Anatomy-DETR criterion.
Built upon DEIM (DETR with Improved Matching for Fast Convergence) and D-FINE.
...
@register()
@register(name='DEIMCriterion')
class AnatomyDETRCriterion(nn.Module):
    """Loss criterion for Anatomy-DETR, with DEIM-compatible behavior."""
    ...
    def loss_labels_focal(self, outputs, targets, indices, num_boxes):
        ...
        loss = loss * self._hard_miss_query_mult(outputs, targets, indices, src_logits).unsqueeze(-1)
    ...
    def loss_labels_mal(self, outputs, targets, indices, num_boxes, values=None):
        ...
        loss = F.binary_cross_entropy_with_logits(src_logits, target_score, weight=weight, reduction='none')
        # #6 up-weight matched queries whose GT falls on systematically-missed tooth-local regions.
        loss = loss * self._hard_miss_query_mult(outputs, targets, indices, src_logits).unsqueeze(-1)
        loss = loss.mean(1).sum() * src_logits.shape[1] / num_boxes
    ...
    def loss_risk_template(self, outputs):
        loss = outputs.get('loss_risk_template', None)
        if loss is None:
            device = outputs['pred_logits'].device
            loss = torch.zeros((), device=device)
        return {'loss_risk_template': loss}
    
    
    def loss_risk_score_prior(self, outputs):
        loss = outputs.get('loss_risk_score_prior', None)
        if loss is None:
            device = outputs['pred_logits'].device
            loss = torch.zeros((), device=device)
        return {'loss_risk_score_prior': loss}


    def loss_risk_reg(self, outputs):
        # #11 L2 anchor that keeps the semi-learnable risk-map residuals near the frozen statistics.
        loss = outputs.get('loss_risk_reg', None)
        if loss is None:
            device = outputs['pred_logits'].device
            loss = torch.zeros((), device=device)
        return {'loss_risk_reg': loss}

    def _hard_miss_query_mult(self, outputs, targets, indices, src_logits):
        """#6 per-query positive-loss multiplier in [1, 1+beta] from the hard-miss map.

        ``outputs['gt_hard_weights']`` is a per-image list of per-GT multipliers attached by the
        decoder (computed under no_grad from the epoch-level hard-miss heatmap).  Matched queries get
        their GT's multiplier; every other query stays at 1.0, so this is a strict no-op whenever the
        decoder did not provide weights (e.g. beta<=0 or before statistics exist).
        """
        gt_weights = outputs.get('gt_hard_weights', None)
        mult = torch.ones(src_logits.shape[:2], device=src_logits.device, dtype=src_logits.dtype)
        if gt_weights is None:
            return mult
        for b, (src_i, tgt_i) in enumerate(indices):
            if b >= len(gt_weights) or gt_weights[b] is None or src_i.numel() == 0:
                continue
            w_b = gt_weights[b].to(device=src_logits.device, dtype=src_logits.dtype).reshape(-1)
            tgt_i = tgt_i.to(src_logits.device).long()
            valid = tgt_i < w_b.numel()
            if valid.any():
                mult[b, src_i.to(src_logits.device).long()[valid]] = w_b[tgt_i[valid]]
        return mult
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                # #6 propagate hard-miss positive-loss multipliers to every intermediate decoder
                # layer so the difficult-region up-weighting applies throughout, not only at the
                # final layer.  Each layer uses its own matching (cached_indices[i]).
                if 'gt_hard_weights' in outputs:
                    aux_outputs['gt_hard_weights'] = outputs['gt_hard_weights']
                for loss in self.losses:
                    # TODO, indices and num_box are different from RT-DETRv2
                    use_uni_set = self.use_uni_set and (loss in ['boxes', 'local'])
        ...
        # Image-conditioned tooth-local risk heatmap supervision is computed exactly once.
        if 'loss_risk_template' in self.weight_dict:
            l_dict = self.loss_risk_template(outputs)
            l_dict = {k: l_dict[k] * self.weight_dict[k] for k in l_dict if k in self.weight_dict}
            losses.update(l_dict)

        if 'loss_risk_score_prior' in self.weight_dict:
            l_dict = self.loss_risk_score_prior(outputs)
            l_dict = {k: l_dict[k] * self.weight_dict[k] for k in l_dict if k in self.weight_dict}
            losses.update(l_dict)

        # #11 semi-learnable risk-map residual L2 anchor (computed exactly once).
        if 'loss_risk_reg' in self.weight_dict:
            l_dict = self.loss_risk_reg(outputs)
            l_dict = {k: l_dict[k] * self.weight_dict[k] for k in l_dict if k in self.weight_dict}
            losses.update(l_dict)
...
DEIMCriterion = AnatomyDETRCriterion
